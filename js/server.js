...
const server = http.createServer((req, res) => {
  if (req.url == "/products" && req.method == "GET") {
    ...
    req.url.match(/\/productsLabel\/([a-z]+)/) &&
    req.method == "GET"
  ) {
    const label = req.url.split("/")[2];
    prodC.getProductbyLabel(req, res, label);
  } else if (
    req.url.match(/\/productsOrigin\/([a-zA-Z]+)/) &&
    req.method == "GET"
  ) {
    ...
    req.url.match(/\/userProfileById\/([0-9]+)/) &&
    req.method == "GET"
  ) {
    const id = req.url.split("/")[2];
    userC.getUserProfileById(req, res, id);
  } else if (
    req.url.match(/\/usersByName\/([a-zA-Z0-9]+)/) &&
    req.method == "GET"
  ) {
    const name = req.url.split("/")[2];
    userC.getUserByUsername(req, res, name);
  } else if (
    req.url.match(/\/usersByEmail\/([a-zA-Z0-9@]+)/) &&
    req.method == "GET"
  ) {
